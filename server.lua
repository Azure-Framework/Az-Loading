...
local function writeStatsFile(tbl)
  ...
  -- ✅ This writes inside the resource folder on the server:
  -- resources/nv_db_loadscreen/html/stats.json
...
-- Keep the old event too (optional live request)
RegisterNetEvent('nv_ls:reqStats', function()
  local src = source
  dbg(('nv_ls:reqStats from %s'):format(src))

  -- Serve the latest file snapshot (fast), but if missing build immediately
  local data = LoadResourceFile(RESOURCE, 'html/stats.json')
  if not data then
    dbg('stats.json missing, building now')
    -- Force a single build/write then read again
    -- (buildAndWrite is defined above)
    buildAndWrite()
    Wait(150)
